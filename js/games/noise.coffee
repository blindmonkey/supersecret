...
require('random/seededrandom.js')
require('simplex.js')
require('three/base')
...
exports.Game = class NoiseGame extends BaseGame
  @loaded: false
  ...
  postinit: ->
    ...
    Params.transform('bg', (n) -> parseInt(n, 16))
    Params.watch('bg', (v) =>
      background.color = v
      setBackground())
    Params.transform('bgo', parseFloat)
    Params.watch('bgo', (v) =>
      background.opacity = v
      setBackground())
    Params.transform('scale', parseFloat)
    Params.watch('scale', (v) =>
      @world.scale = v
      recreateWorld()
    )
    Params.transform('chunksize', parseInt)
    Params.watch('chunksize', (v) =>
      return if not v?
